...
Run once before Phase 1 training. Writes cache/S3/compressed_R*.jsonl files
needed by downstream orchestration contracts.

Usage:
    python preprocess_compress.py              # uses S3 (default)
    python preprocess_compress.py --all        # all split configs (also just S3)
"""
import argparse
import os
...
def main():
    ...
    grp.add_argument("--config", type=str, default="S3",
                     help="Single split config to build cache for (default: S3)")
    ...
    for cfg_id in configs:
        ...
        cache_dir = f"{artifact_root('cache')}/{cfg_id}"
